  ...
  moveit_msgs__msg__CollisionObject__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    moveit_msgs__msg__CollisionObject * data =
      (moveit_msgs__msg__CollisionObject *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!moveit_msgs__msg__CollisionObject__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          moveit_msgs__msg__CollisionObject__fini(&data[i]);
        }
        free(data);
        return false;
      }
