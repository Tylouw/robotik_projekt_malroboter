  ...
  moveit_msgs__msg__PlanningSceneComponents__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    moveit_msgs__msg__PlanningSceneComponents * data =
      (moveit_msgs__msg__PlanningSceneComponents *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!moveit_msgs__msg__PlanningSceneComponents__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          moveit_msgs__msg__PlanningSceneComponents__fini(&data[i]);
        }
        free(data);
        return false;
      }
