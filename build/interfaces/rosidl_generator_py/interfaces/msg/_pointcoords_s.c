...
bool interfaces__msg__pointcoords__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("interfaces.msg._pointcoords.Pointcoords", full_classname_dest, 39) == 0);
  }
  interfaces__msg__Pointcoords * ros_message = _ros_message;
  {  // x
  ...
  {  // curves
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int64_t * dest = ros_message->curves.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // size
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int64_t * dest = ros_message->size.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
{
  /* NOTE(esteve): Call constructor of Pointcoords */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("interfaces.msg._pointcoords");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "Pointcoords");
  ...
  {  // curves
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "curves");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->curves.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int64_t * src = &(ros_message->curves.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->curves.size * sizeof(int64_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // size
    ...
    if (ros_message->size.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int64_t * src = &(ros_message->size.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->size.size * sizeof(int64_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
